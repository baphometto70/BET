...
def update_metrics_file():
    ...
    if not played.empty:
        ...
        denom = pd.to_numeric(played["stake"], errors="coerce").fillna(0).sum()
        ROI = played["ret"].sum()/denom if denom>0 else np.nan

    def _market_roi(df: pd.DataFrame) -> float:
        if df.empty:
            return np.nan
        stakes = pd.to_numeric(df["stake"], errors="coerce").fillna(0)
        if stakes.sum() <= 0:
            return np.nan
        return df["ret"].sum() / stakes.sum()
    ...
    clv_pos_pct = np.nan
    clv_subset = log.copy()
    if not clv_subset.empty:
        clv_subset["quota"] = pd.to_numeric(clv_subset.get("quota"), errors="coerce")
        clv_subset["quota_closing"] = pd.to_numeric(
            clv_subset.get("quota_closing"), errors="coerce"
        )
        clv_subset = clv_subset.dropna(subset=["quota", "quota_closing"])
        if not clv_subset.empty:
            clv_values = (clv_subset["quota_closing"] / clv_subset["quota"]) - 1.0
            clv_values = clv_values.replace([np.inf, -np.inf], np.nan).dropna()
            if not clv_values.empty:
                clv_pos_pct = (clv_values > 0).mean()

    roi_markets = {}
    if not played.empty:
        roi_markets = (
            played.groupby("market_used")
            .apply(_market_roi)
            .to_dict()
        )
    ...
    row = {
        ...
        "n_bet": len(played),
        "ROI_DNB": roi_markets.get("DNB", np.nan),
        "ROI_DC": roi_markets.get("DC", np.nan),
        "ROI_UO": roi_markets.get("O/U", np.nan),
        "ROI_1X2": roi_markets.get("1X2", np.nan)
    }
...
def generate_report_html(outfile="report_scommesse.html", last_n=50):
    ...
    if not clv_series.empty:
        clv_series["quota"] = pd.to_numeric(clv_series["quota"], errors="coerce")
        clv_series["quota_closing"] = pd.to_numeric(clv_series["quota_closing"], errors="coerce")
        clv_series = clv_series.dropna(subset=["quota", "quota_closing"])
        if not clv_series.empty:
            clv_series["clv"] = (clv_series["quota_closing"]/clv_series["quota"]) - 1.0
            clv_values = clv_series["clv"].replace([np.inf, -np.inf], np.nan).dropna()
            if not clv_values.empty:
                fig2 = plt.figure()
                plt.plot(range(len(clv_values)), clv_values.rolling(5).mean())
                plt.title("CLV rolling (5)")
                plt.xlabel("Bet # con CLV")
                plt.ylabel("CLV medio (rolling)")
                img2 = _fig_to_base64(fig2)
                clv_summary_html = (
                    f'<p class="muted">CLV medio: {clv_values.mean():+.3f} '
                    f'— % sopra chiusura: {(clv_values.gt(0).mean()*100):.1f}% '
                    f'(n={len(clv_values)})</p>'
                )
            else:
                img2 = ""
        else:
